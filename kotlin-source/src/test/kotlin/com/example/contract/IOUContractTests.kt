...
class IOUContractTests {
    ...
    @Test
    fun `transaction must include Pay command`() {
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                fails()
                command(listOf(megaCorp.publicKey, miniCorp.publicKey), IOUContract.Commands.Pay())
                verifies()
            }
        }
    }
    ...
    @Test
    fun `transaction must have one input for payment`() {
        ledgerServices.ledger {
            transaction {
                output(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                command(listOf(megaCorp.publicKey, miniCorp.publicKey), IOUContract.Commands.Pay())
                `fails with`("Only one input state should be consumed as payment")
            }
        }
        ...
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                input(IOU_CONTRACT_ID, IOUState(iouValue, megaCorp.party, miniCorp.party))
                output(IOU_CONTRACT_ID, IOUState(-1, miniCorp.party, megaCorp.party))
                command(listOf(megaCorp.publicKey, miniCorp.publicKey), IOUContract.Commands.Pay())
                `fails with`("Only one input state should be consumed as payment")
            }
    ...
    @Test
    fun `pay transaction must have no one output`(){
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                output(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                command(listOf(megaCorp.publicKey, miniCorp.publicKey), IOUContract.Commands.Pay())
                `fails with`("No one output state should be created")
            }
        }
    }

    @Test
    fun `lender is not borrower for payment`() {
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(iouValue, megaCorp.party, megaCorp.party))
                command(listOf(megaCorp.publicKey, miniCorp.publicKey), IOUContract.Commands.Pay())
                `fails with`("The lender and the borrower cannot be the same entity for payment.")
            }
        }
    }

    @Test
    fun `all participants must sign transaction for payment`() {
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                command(megaCorp.publicKey, IOUContract.Commands.Pay())
                `fails with`("All of the participants must be signers.")
            }
        }
        ...
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(iouValue, miniCorp.party, megaCorp.party))
                command(miniCorp.publicKey, IOUContract.Commands.Pay())
                `fails with`("All of the participants must be signers.")
            }
        }
    }
    ...
    @Test
    fun `cannot create negative-value IOUs for payment`() {
        ledgerServices.ledger {
            transaction {
                input(IOU_CONTRACT_ID, IOUState(-1, miniCorp.party, megaCorp.party))
                command(listOf(megaCorp.publicKey, miniCorp.publicKey), IOUContract.Commands.Pay())
                `fails with`("The IOU's value must be non-negative.")
            }
        }
